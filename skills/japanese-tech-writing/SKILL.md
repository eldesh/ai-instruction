...
description: 日本語の技術文書・書籍原稿の文章規範。段落と論証の構成（パラグラフライティング）、論証の厳密さ（ツッコミどころの除去）、読み手の負荷の管理、視点と語り、演出の抑制、LLM っぽい空句の禁止、翻訳調の比喩と擬人化の禁止（「運ぶ」「効く」「開かれた問い」など）、冗長性の排除についての規範を定める。日本語で技術書、記事、解説文を書くとき、または推敲・リライトするときに使用する。
license: Unlicense（https://gist.github.com/k16shikano/67625f2a7d96e3bbdfae8d571a936063）
---

# 日本語技術文書の文章規範

日本語で技術的な内容の原稿（技術書、記事、解説文）を書く・推敲するときは、以下の規範に従う。
...
- 山場で効果を発揮させたい情報（数値、固有の事実）は、その手前の段落で先出ししない。
...
- 同じ概念を二つの対象について対比しながら定義するときは、両方の文の述部を同じ型で揃える。
- 前の説明を振り返る必要があるときは、その振り返りを文頭で済ませ、定義の文自体に「〜だと説明した」を埋め込まない。
- それぞれの特徴を表す語句は「」で示し、二つの定義をつなぐ対比の接続語（「一方」「これに対し」など）を省略しない。
- 第二の対象について続けて挙げる属性は、主語を言い直さず、前の文から地続きに書く。
...
- 読者が順読みで保持する情報は、一段の展開で一つだけ増やす。
- 直前までの文脈から一歩の推論で導ける内容は読者の推論に委ねず、その場で明示的に書く。
...
- 概念を初めて示す文では、読者が順に読んだだけで、まずその概念が何であるか（データ構造、手順、制約などの種類や地位）を読み取れる順序にする。その概念を既に知っている読者にだけ自然な性質・仕組み・制約を、種類の同定より前に置かない。
...
- 例の節の中でも、**その節の問い・帰結に関係しない**過剰な詳細だけを省略する。議論に必要な具体は残す。省略の典型は、エージェント報告の装飾的精度（時刻、HTTP ステータス、カバレッジ率など）や、後で参照しない固有名詞。
...
- 溜め（「ここには〜が潜んでいる」）や修辞疑問で導出を演出するのは、緊張を持たせることが議論の効果を高める要所に限る。説明で足りる箇所では、そのまま述べる。
...
- **予告と総括**：「本章では〜を扱う／探求する」「ここでは〜について見ていく」「まとめると」「要するに」（直前の言い換えだけのとき）、「〜に他ならない」
...
悪い例：「本章では、〇〇の理論を正面から扱う」「この前提を、ここで正面から回収する」。
